    ...
    <div class="card card-main mb-4">
        <div class="card-header d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between">
            ...
            <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center">
                <form action="{{ route('pacientes.index') }}" method="GET" class="form-inline mr-sm-3 mb-2 mb-sm-0 section-search">
                        ...
                        @unless(empty($pacientes))
                            @foreach($pacientes as $paciente)
                                <tr>
                                    ...
                                    <td>{{ $paciente->tiene_ips }}</td>
                                    <td>
                                        <a href="{{ route('pacientes.edit', $paciente->id_paciente) }}" class="btn btn-sm btn-warning">
                                            <i class="fas fa-edit"></i> Editar
                                        </a>
                                        <a href="{{ route('pacientes.historial', $paciente->id_paciente) }}" class="btn btn-sm btn-info" target="_blank" title="Descargar Historial PDF">
                                            <i class="fas fa-file-pdf"></i> PDF
                                        </a>
                                        <form action="{{ route('pacientes.delete', $paciente->id_paciente) }}" method="POST" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
