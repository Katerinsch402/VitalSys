...
<div class="card card-form mb-4">
    <div class="card-header-citas d-flex align-items-center justify-content-between">
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('crear citas')): ?>
            ...
            <table class="table table-hover mb-0" id="tablaCitas">
                ...
                <tbody>
                    <?php if(isset($citas) && count($citas) > 0): ?>
                        ...
                        ?>
                        <tr data-medico-id="<?php echo e($cita->medico_id); ?>">
                            ...
                            <td><?php echo e(\Carbon\Carbon::parse($cita->fec_inicio)->format('d/m/Y H:i')); ?></td>
                            <td><?php echo e(\Carbon\Carbon::parse($cita->fec_fin)->format('d/m/Y H:i')); ?></td>
                            <td>
                                ...
                                <?php elseif($cita->estado == 'Concluido' || $cita->estado == 'concluido'): ?>
                                ...
                                <?php else: ?>
                                    <span class="badge badge-secondary px-2 py-1"><?php echo e($cita->estado); ?></span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?php if($estado == 'Pendiente'): ?>
                                    <button class="btn btn-sm btn-info" title="Reagendar" onclick="reagendarCita(<?php echo e($cita->id_cita); ?>)">
                                        <i class="fas fa-calendar-day"></i>
                                    </button>
                                    <button class="btn btn-sm btn-success" title="Concluir" onclick="concluirCita(<?php echo e($cita->id_cita); ?>)">
                                        <i class="fas fa-check"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger" title="Cancelar" onclick="cancelarCita(<?php echo e($cita->id_cita); ?>)">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content" style="border-radius:16px; overflow:hidden;">
            ...
            <div class="modal-body p-4 position-relative">
                ...
                <form id="formNuevo" action="<?php echo e(route('citas.store')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
...
<script>
    const medicosData = <?php echo json_encode($medicos->map(fn($m) => ['id' => $m->id_medico, 'nombre' => $m->nombre, 'especialidad' => $m->especialidad_id, 'registro' => $m->registro])->values()->all()); ?>;

    const pacientesData = <?php echo json_encode($pacientes->map(fn($p) => ['id' => $p->id_paciente, 'nombre' => $p->nombre, 'apellido' => $p->apellido, 'num_doc' => $p->num_doc])->values()->all()); ?>;

    const salasData = <?php echo json_encode($salas->map(fn($s) => ['id' => $s->id_sala, 'tipo' => $s->tipo_sala, 'numero' => $s->num_sala])->values()->all()); ?>;

    const tipoConsultasData = <?php echo json_encode($tipoConsultas->map(fn($t) => ['id' => $t->id_tipo_consulta, 'descripcion' => $t->descripcion, 'duracion' => $t->duracion])->values()->all()); ?>;
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const flashNotifications = <?php echo json_encode(session('flash_notification'), 15, 512) ?>;
...
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\USUARIO\Desktop\TFG\VitalSys-main\resources\views/citas/index.blade.php ENDPATH**/ ?>